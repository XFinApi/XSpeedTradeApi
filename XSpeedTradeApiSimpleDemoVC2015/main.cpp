/*******************************************************
* XSpeedTradeApiSimpleDemoVC2015
* www.xfinapi.com
*******************************************************/
...
2.拷贝XTA_W32\Api\XSpeed_vX.X.X.X_XXXXXXXX目录（X.X.X.X_XXXXXXXX为版本号）下的所有dll文件到可执行文件目录，
...
{
	printf("  Tick,%s %s, HighestPrice=%g, LowestPrice=%g, BidPrice0=%g, BidVolume0=%lld, AskPrice0=%g, AskVolume0=%lld, LastPrice=%g, TotalVolume=%lld, TradingDay=%s, TradingTime=%s\n",
...
{
	printf("  Balance=%g, Available=%g, CurrMargin=%g, Commission=%g, PositionProfit=%g, CloseProfit=%g, FrozenCash=%g, Withdraw=%g, Deposit=%g\n",
		DEFAULT_FILTER(acc.Balance), DEFAULT_FILTER(acc.Available), DEFAULT_FILTER(acc.CurrMargin), DEFAULT_FILTER(acc.Commission),
		DEFAULT_FILTER(acc.PositionProfit), DEFAULT_FILTER(acc.CloseProfit), DEFAULT_FILTER(acc.FrozenCash), 
...
void MarketTest()
{
	...
	int err = -1;
#ifdef _DEBUG
	market = XFinApi_CreateMarketApi("XTA_W32/Api/XSpeed_v1.5.1.9_20170830/XFinApi.XSpeedTradeApid.dll", &err);
#else
	market = XFinApi_CreateMarketApi("XTA_W32/Api/XSpeed_v1.5.1.9_20170830/XFinApi.XSpeedTradeApi.dll", &err);
#endif
...
void TradeTest()
{
	...
	int err = -1;
#ifdef _DEBUG
	trade = XFinApi_CreateTradeApi("XTA_W32/Api/XSpeed_v1.5.1.9_20170830/XFinApi.XSpeedTradeApid.dll", &err);
#else
	trade = XFinApi_CreateTradeApi("XTA_W32/Api/XSpeed_v1.5.1.9_20170830/XFinApi.XSpeedTradeApi.dll", &err);
#endif
	if (err || !trade)
	{
