/*******************************************************
* XSpeedTradeApiSimpleDemoLinux64GCC
* www.xfinapi.com
*******************************************************/
...
/***************************************************
Ubuntu中缺失libssl.so.10的解决办法

1.先保证SSL已经被安装
sudo apt-get update
sudo apt-get install libssl1.0.0 libssl-dev12
...
{
	printf("  Tick,%s %s, HighestPrice=%g, LowestPrice=%g, BidPrice0=%g, BidVolume0=%ld, AskPrice0=%g, AskVolume0=%ld, LastPrice=%g, TotalVolume=%ld, TradingDay=%s, TradingTime=%s\n",
...
void MarketTest()
{
	...
	market = XFinApi_CreateMarketApi("XTA_L64/Api/XSpeed_v1.5.1.9_20170830/XFinApi.XSpeedTradeApi.so", &err);
...
void TradeTest()
{
	...
	trade = XFinApi_CreateTradeApi("XTA_L64/Api/XSpeed_v1.5.1.9_20170830/XFinApi.XSpeedTradeApi.so", &err);
